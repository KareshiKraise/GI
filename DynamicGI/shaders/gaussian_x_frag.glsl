...
void main()
{
	...
	if (edge == 0)
	{
		...
		{
			edge = texture(edgeBuffer, (vec2(gl_FragCoord) + vec2(offset[i], 0.0)) / vec2(Wid, Hei)).x;
			if (edge == 1.0)
			{
				break;
			}
			...
			{
				num_samp += 1;
				color += texture(lightingBuffer, (vec2(gl_FragCoord) + vec2(offset[i], 0.0)) / vec2(Wid, Hei)) * weight[i];
				w += weight[i];
			}
		}
	}
