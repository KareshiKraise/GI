...
vec3 calculate_indirect(vec3 P, vec3 N) {
	int M = 0;

	vec4 l_space_pos = LightSpaceMat * vec4(P, 1.0);
	...
	for (int i = 0; i < N_samples; i++) {
		vec2 n_tex = get_spiral_texel(proj.xy, i);
...
void main() {
	...
	vec3 indirect = calculate_indirect(pos, normal);
